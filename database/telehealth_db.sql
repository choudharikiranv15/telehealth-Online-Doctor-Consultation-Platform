...
-- Includes: Ratings, Password Reset, and Reschedule Features
-- ===================================================================
...
CREATE TABLE doctor_profiles (
    ...
    available_days VARCHAR(255) DEFAULT 'monday,tuesday,wednesday,thursday,friday,saturday' COMMENT 'Comma-separated list of available days',
...
-- ===================================================================
-- 5. APPOINTMENTS TABLE (Core booking system with reschedule and missed call tracking)
-- ===================================================================
CREATE TABLE appointments (
    ...
    status ENUM('pending', 'confirmed', 'active', 'completed', 'cancelled', 'no_show', 'reschedule_requested', 'rejected', 'missed') DEFAULT 'pending',
    booking_type ENUM('consultation', 'follow_up', 'emergency') DEFAULT 'consultation',
    symptoms TEXT NOT NULL,
    notes TEXT COMMENT 'Patient additional notes',
    doctor_notes TEXT COMMENT 'Doctor notes during consultation',
    ...
    requested_date DATE NULL COMMENT 'Patient requested reschedule date',
    requested_time TIME NULL COMMENT 'Patient requested reschedule time',
    original_date DATE NULL COMMENT 'Original appointment date before reschedule',
    original_time TIME NULL COMMENT 'Original appointment time before reschedule',
    reschedule_reason TEXT NULL COMMENT 'Patient reason for rescheduling',
    reschedule_requested_at TIMESTAMP NULL COMMENT 'When reschedule was requested',
    reschedule_response TEXT NULL COMMENT 'Doctor response to reschedule request',
    rejection_reason TEXT NULL COMMENT 'Doctor reason for rejecting appointment or reschedule',
    reviewed_by INT NULL COMMENT 'Doctor who reviewed the appointment',
    reviewed_at TIMESTAMP NULL COMMENT 'When the appointment was reviewed',
    missed_by VARCHAR(20) NULL COMMENT 'Who missed the appointment: doctor, patient, or NULL',
    updated_by INT NULL COMMENT 'User who last updated the appointment',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    INDEX idx_reschedule_status (status, requested_date),
    INDEX idx_missed_by (missed_by),
    INDEX idx_status_date (status, appointment_date)
);

-- ===================================================================
-- 6. CALL SESSIONS TABLE (Track when users join video calls)
-- ===================================================================
...
-- ===================================================================
-- 7. VIDEO CALLS TABLE (Jitsi integration)
-- ===================================================================
CREATE TABLE video_calls (
    ...
    participants_joined JSON COMMENT 'Track who joined and when',
    quality_rating INT DEFAULT 0 COMMENT '1-5 rating',
...
-- ===================================================================
-- 8. PRESCRIPTIONS TABLE (Medical prescriptions)
-- ===================================================================
...
-- ===================================================================
-- 9. REVIEWS TABLE (Doctor ratings and feedback)
-- ===================================================================
...
-- ===================================================================
-- 10. PASSWORD RESET TOKENS TABLE (Password recovery system)
-- ===================================================================
...
CREATE TABLE notifications (
    ...
    related_id INT COMMENT 'ID of related appointment, payment, etc.',
...
-- ===================================================================
-- 12. WEBRTC SIGNALS TABLE (WebRTC signaling for video calls)
-- ===================================================================
CREATE TABLE webrtc_signals (
    ...
    receiver_id INT,
    type VARCHAR(50) NOT NULL COMMENT 'Signal type: offer, answer, ice-candidate',
    data TEXT NOT NULL COMMENT 'JSON data for the signal',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- ===================================================================
-- 13. APPOINTMENT HISTORY TABLE (Track all appointment changes)
-- ===================================================================
...
-- ===================================================================
-- 14. PAYMENTS TABLE (Payment transaction tracking)
-- ===================================================================
...
-- ===================================================================
-- 15. SYSTEM SETTINGS TABLE (App configuration)
-- ===================================================================
...
INSERT INTO doctor_profiles (user_id, specialization_id, license_number, experience_years, qualification, consultation_fee, bio, languages, available_days, rating, total_reviews, is_verified) VALUES
(2, 1, 'MED001', 10, 'MBBS, MD (General Medicine)', 600.00, 'Experienced general practitioner with 10 years of clinical experience. Specialized in treating common health conditions and preventive care.', 'English, Hindi, Marathi', 'monday,tuesday,wednesday,thursday,friday,saturday', 4.5, 120, TRUE),
(3, 2, 'MED002', 8, 'MBBS, DM (Cardiology)', 1200.00, 'Cardiologist with expertise in heart conditions, ECG interpretation, and cardiovascular health management.', 'English, Hindi', 'monday,tuesday,wednesday,thursday,friday', 4.7, 95, TRUE),
(4, 3, 'MED003', 6, 'MBBS, MD (Dermatology)', 800.00, 'Dermatologist specializing in skin conditions, acne treatment, and cosmetic dermatology.', 'English, Hindi, Gujarati', 'monday,wednesday,friday,saturday', 4.3, 78, TRUE),
(5, 4, 'MED004', 12, 'MBBS, MD (Pediatrics)', 700.00, 'Pediatrician with 12 years of experience in child healthcare, vaccinations, and developmental issues.', 'English, Hindi, Tamil', 'monday,tuesday,wednesday,thursday,friday,saturday', 4.8, 150, TRUE),
(6, 1, 'MED005', 5, 'MBBS, MS (General Surgery)', 900.00, 'General practitioner with surgical experience. Provides comprehensive healthcare services.', 'English, Hindi, Punjabi', 'tuesday,thursday,saturday', 4.4, 62, TRUE);
...
-- Update doctor rating when review is updated
...
CREATE TRIGGER update_doctor_rating_after_delete
...
-- Generate prescription number automatically
CREATE TRIGGER generate_prescription_number
BEFORE INSERT ON prescriptions
FOR EACH ROW
BEGIN
    DECLARE next_num INT;
    SELECT COALESCE(MAX(CAST(SUBSTRING(prescription_number, 4) AS UNSIGNED)), 0) + 1
    INTO next_num
    FROM prescriptions
    WHERE prescription_number LIKE 'RX_%';

    SET NEW.prescription_number = CONCAT('RX_', LPAD(next_num, 6, '0'));
END//
...
CREATE FULLTEXT INDEX idx_doctor_search ON doctor_profiles(bio, qualification);

-- ===================================================================
-- FINAL SUCCESS MESSAGE
-- ===================================================================
SELECT 'Complete TeleHealth database created successfully!' as Status,
       'Base + Ratings + Password Reset + Reschedule + Missed Call + WebRTC + Payments' as Features,
       ...
       'All 15 tables created with complete schema' as DatabaseInfo;
